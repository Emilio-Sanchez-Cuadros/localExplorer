...
class Displayer extends React.Component {
    ...
    async getFiles() {
        ...
        const response = await fetch(`${URL}${this.props.path}`, {
            headers: {
    ...
    render() {
        const { currentPage, files, folderIcon, fileIcon } = this.state;
                ...
                <div className="inside-window-2 p-5">
                    <ul>
                        {files.slice(filesToShowPosition, filesToShowPosition + filesPerPage).map((file) => (
                            <li key={file}>{file.includes(".") ? <img src={fileIcon} style={{width : '23px'}}></img> : <img src={folderIcon} style={{width : '23px'}}></img>}
                            {file}</li>
                        ))}
                    </ul>
                </div>
